...
def enemies(x, y, z, type="normal", health=1):
    glPushMatrix()
...
def update_enemies():
    ...
    for t in targets[:]:
        ...
        if dist < 50:
            ...
            ang = math.atan2(dy, dx)
            speed = target_speed * 0.3 if t[3] == "boss" else target_speed
...
def detect_target_hits():
    global shots, targets, player_score, enemies_killed, coins
    if round_pause:
        return
    for s in shots[:]:
        s_x, s_y = s[0], s[1]
...
def tower_shoot(tower_idx, tx, ty, level=1):
    global tower_shots, targets
    cooldown = tower_shot_cooldown * 0.7 if level >= 2 else tower_shot_cooldown
    damage = tower_shot_damage * 2 if level == 3 else tower_shot_damage
    closest_enemy = None
    ...
    for t in targets:
        ...
        if dist < min_dist:
            ...
            closest_enemy = t
    if closest_enemy:
        ex, ey, ez, etype, ehealth = closest_enemy
        dx = ex - tx
        dy = ey - ty
        ang = math.atan2(dy, dx)
        ang += random.uniform(-0.05, 0.05)
        tower_shots.append([tx, ty, 160, ang, damage])
        return True
    return False
...
def update_towers():
    ...
    if round_pause:
        return
    for i, (tx, ty, level) in enumerate(towers):
        cooldown = tower_shot_cooldown * 0.7 if level >= 2 else tower_shot_cooldown
        if i in tower_shot_timers:
            tower_shot_timers[i] -= 1
            if tower_shot_timers[i] <= 0:
                if tower_shoot(i, tx, ty, level):
                    tower_shot_timers[i] = cooldown
                else:
                    tower_shot_timers[i] = 60
        else:
            tower_shot_timers[i] = random.randint(60, int(cooldown))
...
def update_tower_shots():
    ...
    for shot in tower_shots:
        ...
        for t in targets:
            ...
            dy = shot[1] - t[1]
            dist = math.sqrt(dx*dx + dy*dy)
            damage = shot[4] if len(shot) > 4 else tower_shot_damage
            if dist < 40:
                player_score += 1
                enemies_killed += 1
                to_remove_shots.append(shot)
...
def spawn_enemies(count=target_number):
    ...
    for _ in range(count):
        ...
        is_boss = (current_round % 3 == 0) and (len(targets) == 0)
        if is_boss:
            targets.append([x, y, z, "boss", 10])
        else:
            targets.append([x, y, z, "normal", 1])
...
def reset_game():
    ...
    global tower_shots, tower_shot_timers, round_pause, round_choice_made, kills_to_advance, enemies_killed, coins
    ...
    castle_radius = 60
    kills_to_advance  = 10
    gun_rotation, player_health, player_max_health, player_score, misses, coins = 180, 100, 100, 0, 0, 100
    shots.clear()
    targets.clear()
    tower_shots.clear()
...
def keyboardListener(key, x, y):
    ...
    global round_pause, round_choice_made, towers, tower_shot_timers, tower_placement_mode, placement_marker_position, coins
    if round_pause:
        if tower_placement_mode:
            if key == b's' and placement_marker_position[1] < region - 50:
                ...
                if (abs(placement_marker_position[0]) > castle_region or
                    ...
                    if len(towers) < 5 and coins >= 50:
                        towers.append(tuple(placement_marker_position) + (1,))
    ...
    if game_end and key != b'r':
        ...
        if towers:
            ...
            for i, (tx, ty, level) in enumerate(towers):
                if level < 3:
                    dist = math.sqrt((tx - player_position[0])**2 + (ty - player_position[1])**2)
                    if dist < min_dist:
                        min_dist = dist
                        nearest_idx = i
            if nearest_idx >= 0 and coins >= 30:
                tx, ty, level = towers[nearest_idx]
                towers[nearest_idx] = (tx, ty, level + 1)
                coins -= 30
    if key == b'p':
        player_health = 1000
    if not cheat:
...
def idle():
    ...
    if round_pause:
        ...
        return
    if not game_end:
        if cheat and targets:
            t = targets[0]
            ang = math.degrees(math.atan2(t[1] - player_position[1], t[0] - player_position[0]))
            gun_rotation = (ang + 360) % 360
            shoot()
        update_enemies()
        update_enemy_shots()
        update_towers()
...
def showScreen():
    ...
    if round_pause:
        ...
        draw_text(10, 650 - 115, f"Coins: {coins}", color=(1, 0.8, 0))
